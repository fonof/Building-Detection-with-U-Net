...
| **GPU** | NVIDIA с CUDA |
| **VRAM** | ~8 GB |
| **Batch size** | 8 (как в обучении) |
| **Python** | 3.10+ |
| **ОС** | Windows / Linux |

Обучение на CPU возможно, но будет **очень медленным** — рекомендуется GPU.

## ⚠️ Ограничения

- Обучение на **подвыборке 10 000 патчей**, не на полном Inria (~180k патчей после нарезки)
- Val IoU **0.77** — хороший baseline; на full dataset обычно выше (~0.85+)
- Модель заточена под **патчи 256×256** — для больших снимков нужен sliding window
- Веса через **Git LFS** — при клоне нужны `git lfs install` и `git lfs pull`
